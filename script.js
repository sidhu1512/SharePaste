...
// Themes
const themes = [
    { name: 'tomorrow', url: 'https://cdnjs.cloudflare.com/ajax/libs/prism/1.29.0/themes/prism-tomorrow.min.css', bg: '#1e1e22' },
...
async function main() {
    ...
    if (iconEl) {
        iconEl.href = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32' width='32' height='32'%3E%3Crect width='32' height='32' rx='8' fill='%23050505'/%3E%3Crect x='1' y='1' width='30' height='30' rx='7' fill='none' stroke='%2327272a' stroke-width='1'/%3E%3Cpath d='M11 11L6 16L11 21' fill='none' stroke='%23e4e4e7' stroke-width='2.5' stroke-linecap='round' stroke-linejoin='round'/%3E%3Cpath d='M21 11L26 16L21 21' fill='none' stroke='%23e4e4e7' stroke-width='2.5' stroke-linecap='round' stroke-linejoin='round'/%3E%3Cline x1='18' y1='9' x2='14' y2='23' stroke='%2371717a' stroke-width='2' stroke-linecap='round'/%3E%3C/svg%3E";
    }
...
function detectLanguage(code) {
    ...
    if (
        ...
        /\b(public|private|protected)\s+(static\s+)?(final\s+)?(class|interface|enum|void|int|long|boolean|double|float|char|String)\b/.test(code) ||
        (/\bclass\s+\w+/.test(code) && code.includes('{') && code.includes(';') && !code.includes('def '))
    ) {
        return 'java';
    }
    ...
    if (/^\s*func\s+(\([^)]+\)\s+)?\w+\s*\(/.test(code) || /\bfmt\.(Print|Println|Printf|Sprintf)\b/.test(code)) return 'go';

    // 5. HTML / XML
    if (/<!DOCTYPE\s+html/i.test(code) || (/<[a-z][\s\S]*>/i.test(code) && (code.includes('</') || code.includes('/>')))) return 'markup';
    ...
    const hasPyImport = /^\s*(import\s+[a-zA-Z_]\w*(\s*,\s*[a-zA-Z_]\w*)*|from\s+[a-zA-Z_]\w*\s+import\s+[\w*,\s()]+)$/m.test(code);
    ...
    if (hasPyDef || hasPyClass || (hasPyImport && !code.includes(';')) || hasPyControl || hasPyDunder || (hasPyPrint && !code.includes(';'))) {
        return 'python';
    }

    // 7. TypeScript
    if (/\b(interface\s+\w+\s*\{|type\s+\w+\s*=|as\s+const\b|:\s*(string|number|boolean|any|void|unknown|never)\b)/.test(code)) return 'typescript';

    // 8. CSS
    if (/^[\s\w.-]+\s*\{[^}]*[\w-]+\s*:\s*[^;]+;/m.test(code) || /@(media|keyframes)\s+/.test(code)) return 'css';

    // 9. SQL
    if (/^\s*(SELECT\s+[\w*,\s]+\s+FROM|INSERT\s+INTO|UPDATE\s+\w+\s+SET|DELETE\s+FROM|CREATE\s+TABLE|ALTER\s+TABLE)\b/im.test(code)) return 'sql';

    // 10. JSON
    if (/^\s*\{\s*"[\w-]+"\s*:\s*/.test(code) || /^\s*\[\s*\{\s*"/.test(code)) return 'json';

    // 11. Bash / Shell
    if (/^#!\/bin\/(bash|sh|zsh)/.test(code) || /^\s*(sudo\s+|apt\s+|npm\s+run|git\s+commit|chmod\s+\+x)/m.test(code)) return 'bash';
...
function updateUrl(saveToHistory = false, currentLines = 0) {
    ...
    try {
        ...
        const linkLen = window.location.origin.length + window.location.pathname.length + 1 + base64.length;
        const compressionRatio = charCount > 0 ? Math.round((1 - (compressed.length / buffer.length)) * 100) : 0;
        const ratioText = compressionRatio > 0 ? ` · ${compressionRatio}% SAVED` : '';
        const urlPillClass = linkLen >= QR_MAX_URL_LENGTH ? "text-zinc-400 font-medium" : "text-zinc-400";
...
if (downloadBtn) {
    downloadBtn.addEventListener('click', (e) => {
        ...
        const extMap = {
            ...
            go: 'go', cpp: 'cpp', java: 'java', html: 'html', markup: 'html',
            css: 'css', json: 'json', sql: 'sql', markdown: 'md', bash: 'sh'
        };
    ...
    if (files.length > 0) {
        ...
        reader.onload = (event) => {
            ...
            const extToLang = {
                ...
                rs: 'rust', go: 'go', cpp: 'cpp', c: 'cpp', h: 'cpp', hpp: 'cpp',
                java: 'java', html: 'markup', xml: 'markup', css: 'css', json: 'json',
                sql: 'sql', md: 'markdown', sh: 'bash', bash: 'bash', txt: 'none'
            };
            if (extToLang[ext]) {
                manualLanguage = extToLang[ext];
                activeLanguage = manualLanguage;
                if (langLabel) langLabel.textContent = langDisplayMap[manualLanguage] || manualLanguage.toUpperCase();
            }
